...
function charmSuffixForFraction(frac: number) {
  if (frac < 0.5) return 0.49;
  if (frac < 0.85) return 0.89;
  return 0.99;
}

export function applyCharmPricing(price: number) {
...
export function priceFromCost(cost: number, markupPercent: number, charmPricing = false) {
