...
import { countActiveOffers, getVoucherGames } from '../lib/catalogUtils';

const GIFT_CARDS_PREVIEW = 9;
...
export default function GiftCardsView({
  ...
  const isAr = lang === 'ar';
  const voucherGames = getVoucherGames(games)
    .map((game) => ({
      ...game,
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {voucherGames.slice(0, GIFT_CARDS_PREVIEW).map((game) => (
            <HomeGameCard
              key={game.id}
              game={game}
              lang={lang}
      ...
      {!loading && voucherGames.length > GIFT_CARDS_PREVIEW && (
        <p className="text-center text-sm text-[var(--text-muted)] mt-6">
          {isAr
            ? `+${voucherGames.length - GIFT_CARDS_PREVIEW} فئات أخرى في الصفحة الرئيسية`
            : `+${voucherGames.length - GIFT_CARDS_PREVIEW} more categories on the home page`}
        </p>
      )}
    </div>
  );
