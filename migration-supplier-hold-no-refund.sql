-- Hold (do not refund) balance orders when supplier wallet is too low.
-- Re-creates apply_g2bulk_fulfillment with the awaiting_funds hold path.
...
BEGIN
  ...
  THEN
    IF v_order.payment_method = 'balance'
       AND COALESCE((v_meta->>'reason')::text, '') = 'insufficient_supplier_balance'
    THEN
      -- Supplier wallet too low: HOLD, do not refund. Balance stays debited;
      -- admin tops up G2Bulk and retries fulfillment. Prevents refund-wipe of
      -- paid orders and the free-re-fulfill hazard after the supplier completes.
      v_meta := v_meta || jsonb_build_object(
        'awaiting_funds', true,
        'awaiting_funds_at', now()
      );
