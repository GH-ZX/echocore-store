-- Reverts the awaiting_funds hold in apply_g2bulk_fulfillment.
-- The admin toggle g2bulk_block_when_wallet_low (ON by default) now prevents
-- low-wallet orders at checkout; if a fulfillment still fails, the order is
-- simply failed and the customer balance is refunded (old behavior).
-- No more holds ("if failed then it failed").
